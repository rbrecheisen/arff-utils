...
coverage:
	coverage run --source arff_utils setup.py test
	coverage report -m
	coverage html
	@echo "To view results type: htmlcov/index.html &"

docs:
	rm -f docs/arff_utils.rst
	rm -f docs/modules.rst
	sphinx-apidoc -f -o docs/ arff_utils
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
install: dist
	pip install --upgrade --no-deps --force-reinstall dist/arff_utils-*.tar.gz

uninstall: 
	pip uninstall -y arff_utils
